...
async def run_test():
    ...
    try:
        ...
        # -> Log in as Admin to test access to protected routes and role restrictions.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Super Admin Exclusive Access Granted').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test failed: JWT token enforcement and role-based access control verification failed as per the test plan. This assertion fails immediately to indicate the test plan execution failure.')
