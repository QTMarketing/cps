...
async def run_test():
    ...
    try:
        ...
        # -> Verify credentials or try to recover/reset password or check for alternative login method.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Bank created successfully').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: Bank creation should fail when any mandatory field is missing, but the success message 'Bank created successfully' was not found as expected.")
