...
async def run_test():
    ...
    try:
        ...
        # -> Click on Login button to initiate login as admin user.
        frame = context.pages[-1]
        # Click Login button to start login process as admin user
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Bank Creation Successful').first).to_be_visible(timeout=3000)
        except AssertionError:
            raise AssertionError('Test failed: Bank creation did not fail as expected when routing number is not exactly 9 digits. The form validation should prevent submission and display a routing number format error.')
