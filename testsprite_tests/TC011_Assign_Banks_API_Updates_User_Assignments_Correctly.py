...
async def run_test():
    ...
    try:
        ...
        # -> Click on the Login button to authenticate with valid credentials.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Bank Assignment Successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: The API to assign banks to a user did not update the assigned_to_user_id field correctly or did not enforce security as expected.")
