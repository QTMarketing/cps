...
async def run_test():
    ...
    try:
        ...
        # -> Login with provided credentials to gain access to the Add-User page.
        frame = context.pages[-1]
        # Click Login button to open login form or login page
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Unassigned Banks API call successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: The unassigned banks API endpoint did not return the expected data with only banks having null assigned user IDs including id, dbaName, accountType, and bankName.')
