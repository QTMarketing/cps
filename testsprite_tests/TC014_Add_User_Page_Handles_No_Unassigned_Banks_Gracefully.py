...
async def run_test():
    ...
    try:
        ...
        # -> Login with admin credentials to gain access to Add User page.
        frame = context.pages[-1]
        # Click Login button to open login form or page.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Unassigned banks available for assignment').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: The Add User page did not display the expected message indicating no unassigned banks are available, or bank selection checkboxes were not disabled as required by the test plan.")
