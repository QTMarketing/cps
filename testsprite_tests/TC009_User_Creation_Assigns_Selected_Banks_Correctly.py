...
async def run_test():
    ...
    try:
        ...
        # -> Login with admin credentials to gain access to the Add User page.
        frame = context.pages[-1]
        # Click the Login button to go to login page or login form
        ...
        frame = context.pages[-1]
        # Input username admin@quicktrackinc.com
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=User creation failed due to unassigned banks not being assigned').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: The user creation process did not assign the selected unassigned banks to the new user as expected.")
