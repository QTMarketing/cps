...
async def run_test():
    ...
    try:
        ...
        # -> Click on Login button to attempt login with provided credentials.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Cheque Printing Successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: The cheque printing process did not complete successfully. The output did not include the bank data, digital signature image, auto-incremented cheque numbers, or correct return address as required by the test plan.")
