...
export default function WriteChecksClient({ token }: Props) {
    ...
    const load = async () => {
      try {
        ...
        const [b, v, s, c] = await Promise.all([
          ...
          fetch("/api/checks", { headers: authHeaders }).then(r => r.ok ? r.json() : [])
        ]);
  ...
  const submit = async () => {
    if (!vendorId || !storeId || !amount) { alert("Please complete the form"); return; }
    if (!invoiceUrl) { alert("Invoice is required"); return; }
    try {
      setLoading(true);
      const body = {
      ...
      alert(`Failed to create cheque: ${e?.message || e}`);
    } finally { setLoading(false); }
  };
